...
const projectAlertTitle = 'You have an unsaved test!'

export const projectAlertDialogId = uid()
...
const actions: ActionTree<TestSuiteDetail, RootState> = {
  setId({ commit, state, rootState }, id: string) {
    ...
    const hasUnfinishedTest =
      ...
      !state.created &&
      id !== state.id &&
      !!rootState.projectSuite.create.section.items.length
            ...
            actions: [
              {
                ...
                events: {
                  click: () => {
                    ...
                    app.$store.dispatch('projectSuite/create/resetForm')
