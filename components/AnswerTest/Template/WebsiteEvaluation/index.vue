...
export default defineComponent({
  ...
  components: { Dialog, Spinner, FadeTransition },
  setup(_, { root }) {
...
<template>
  <div class="relative isolate h-full">
    <div
      class="h-76 w-full shadow-divide-header relative mb-8 z-1 font-sf-pro-display font-semibold leading-[32px] text-[20px] flex-centered"
    >
      {{
        currentSection.task ||
    ...
    <div class="max-h-full overflow-auto h-[calc(100%-84px)]">
      <div class="w-full h-full pointer-events-none relative">
        <!-- iframe -->
        <div
          v-if="!removeIFrame && !iframeErrorReason"
        ...
        >
          ...
          <iframe
            ...
            :class="{
              'pointer-events-auto': dialogEntered && !draggingDialog,
              invisible: !iframeLoadSuccess,
            }"
            @load="onLoad"
          />
        </div>
