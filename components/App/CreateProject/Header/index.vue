...
export default defineComponent({
  ...
  setup(_, { root }) {
    ...
    const testTitle = computed({
      ...
      set(val: string) {
        if (typeof val === 'string') {
          root.$store.commit('projectSuite/detail/setData', {
            name: val,
          })
        }
      },
    ...
    const enableEditing = computed(() => {
      return root.$route.name === 'dashboard-project-:id'
    })

    const emptyTestForm = computed(() => {
      const state = (root.$store.state as RootState).projectSuite
...
<template>
  <div>
    <div class="flex items-center">
      ...
      >
        ...
        >
          <PBadge
            v-if="
              ($route.name === 'dashboard-project-:id' && !emptyTestForm) ||
              $route.name === 'dashboard-project-recruit-:id' ||
              $route.name.startsWith('dashboard-project-view-result-')
            "
    ...
    <FadeTransition>
      <div
        v-if="$route.name === 'dashboard-project-:id'"
