...
export default defineComponent({
  name: 'AppProjectStepsWelcomeScreen',
  components: { Section },
  emits: ['on-update'],
  setup(_, { root: { $store } }) {
      ...
      return new Proxy(
        {},
        {
          ...
          set(_, path: keyof typeof storeValue, value: any) {
            $store.commit('projectSuite/create/welcomeScreen/setData', {
              [path]: value,
            })
...
<template>
  <Id id="project-welcome-screen" v-slot="{ id }">
    <Section
      :id="id"
    ...
    >
      <h4 class="text-text-subdued mb-20">
        This is the welcome screen your participants first see. You can also add
