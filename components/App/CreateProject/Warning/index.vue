...
export default defineComponent({
  ...
  setup(_, { root }) {
    ...
    const showWarning = computed(() => {
      return (
        root.$route.name === 'dashboard-project-:id' && state.value.create.showWarning
      )
    })
    ...
    const duplicateTest = async () => {
      await root.$store.dispatch('projectSuite/create/duplicate')
    }

    const continueTest = () => {
      root.$store.commit('projectSuite/create/setShowWarning', false)
    }
