<template>
  <section class="bg-surface-default shadow-2 p-20 rounded-lg w-full">
    <div class="flex items-center justify-between mb-20">
      <h3 class="text-heading font-semibold">Card Details</h3>

      <Button @click="showDetails = !showDetails"> Add payment method </Button>
    </div>
      ...
      <ul v-else class="grid gap-y-20">
        <li v-for="(card, i) in cards" :key="card.number">
          <button
            ...
            :class="{
              'bg-[rgba(38,125,255,0.05)] border-focused-default':
                i === activeCard,
          ...
          >
            ...
            >
              <Img
                src="static/png/app/Settings/Billing/cards/visa"
                :width="31"
                alt="Visa icon"
                class="min-w-31"
              />
            ...
            <div class="flex-grow mr-20 text-left">
              ...
              <p>Visa card{{ card.default ? ' - Selected as default' : '' }}</p>
            </div>

            <p class="mr-20">Expiry {{ card.expires }}</p>

            <Button plain> Edit </Button>

            <div class="ml-64">
              <div
                class="w-18 h-18 rounded-full border-2 flex-centered before:block before:rounded-full before:w-10 before:h-10 before:bg-interactive-default before:transform-gpu before:transition-transform transition-[border-color]"
                :class="{
                  'before:scale-0 border-border-default': activeCard !== i,
                  'border-interactive-default': activeCard === i,
                }"
              />
            </div>
          </button>
